...
const AdminMostSell = () => {
  ...
  const colorOptions = ["white", "black", "blue", "glass"]; // گزینه‌های رنگ
  ...
  const handleDelete = async (id: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:3001/MostSell/${id}`);
  ...
  const handleEdit = (item: MostSellItem) => {
    setEditItem(item);
    setIsModalOpen(true);
  };
  ...
  const handleUpdate = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.put(
            `http://localhost:3001/MostSell/${editItem.id}`,
            editItem
          );
  ...
  const handleAddItem = async () => {
    ...
    const newItemWithId = { ...newItem, id: crypto.randomUUID() }; // تولید آیدی جدید

    try {
      await axios.post("http://localhost:3001/MostSell", newItemWithId);
